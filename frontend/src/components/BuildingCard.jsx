...
const BuildingCard = ({ building, isSelected, onSelect }) => {
  ...
  const getStatusIcon = () => {
    if (building.status.toLowerCase() === 'critical' || building.status.toLowerCase() === 'warning') {
      return (
        <ExclamationTriangleIcon
          className={`h-5 w-5 ${
            building.status.toLowerCase() === 'critical' ? 'text-red-500' : 'text-yellow-500'
          }`}
          aria-hidden="true"
        />
      );
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-lg font-semibold text-gray-900">
            {building.currentUsage} <span className="text-sm font-normal text-gray-500">kW</span>
          </p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Today</p>
          <p className="text-lg font-semibold text-gray-900">
            {building.dailyUsage} <span className="text-sm font-normal text-gray-500">kWh</span>
          </p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Devices</p>
          <p className="text-lg font-semibold text-gray-900">{building.deviceCount}</p>
        </div>
